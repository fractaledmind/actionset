...
RSpec.describe 'GET /foos with FILTERING', type: :request do
  ...
  context 'with a :calculated_boolean_field attribute' do
    context 'on the base object' do
      let(:params) do
        { filter: { calculated_boolean_field: foo.calculated_boolean_field } }
      end
      let(:expected_ids) do
        Foo.all
           .select { |x| x.calculated_boolean_field == foo.calculated_boolean_field }
           .map(&:id)
      end
    ...
    context 'on an associated object' do
      let(:params) do
        { filter: { assoc: { calculated_boolean_field: foo.assoc.calculated_boolean_field } } }
      end
      let(:expected_ids) do
        Foo.all
           .select { |x| x.assoc.calculated_boolean_field == foo.assoc.calculated_boolean_field }
           .map(&:id)
      end
