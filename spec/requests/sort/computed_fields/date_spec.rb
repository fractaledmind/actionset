...
RSpec.describe 'GET /foos with SORTING', type: :request do
  ...
  context 'with a :calculated_date_field attribute' do
    context 'on the base object' do
      ...
      context 'when direction is ASC' do
        ...
        context 'with default case-sensitive sort' do
          let(:sort_param) { { 'calculated_date_field': direction } }
          let(:expected_ids) do
            Foo.all
               .sort_by(&:calculated_date_field)
               .map(&:id)
          end
        ...
        context 'with case-INsensitive sort' do
          let(:sort_param) { { 'calculated_date_field(i)': direction } }
          let(:expected_ids) do
            Foo.all
               .sort_by { |x| x.calculated_date_field.to_s.downcase }
               .map(&:id)
          end
    ...
    context 'on an associated object' do
      ...
      context 'when direction is ASC' do
        ...
        context 'with default case-sensitive sort' do
          ...
          let(:expected_ids) do
            Foo.all
               .sort_by { |x| x.assoc.calculated_date_field }
               .map(&:id)
          end
        ...
        context 'with case-INsensitive sort' do
          ...
          let(:expected_ids) do
            Foo.all
               .sort_by { |x| x.assoc.calculated_date_field.to_s.downcase }
               .map(&:id)
          end
