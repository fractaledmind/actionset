    ...
    context 'on the base object' do
      ...
      context 'when direction is ASC' do
        ...
        context 'with default case-sensitive sort' do
          let(:sort_param) { { 'datetime': direction } }
          let(:expected_ids) do
            Foo.order(datetime: direction)
               .pluck(:id)
          end
        ...
        context 'with case-INsensitive sort' do
          let(:sort_param) { { 'datetime(i)': direction } }
          let(:expected_ids) do
            Foo.order("LOWER(datetime) #{direction}")
               .pluck(:id)
          end
    ...
    context 'on an associated object' do
      ...
      context 'when direction is ASC' do
        ...
        context 'with default case-sensitive sort' do
          ...
          let(:expected_ids) do
            Foo.joins(:assoc)
               .merge(Assoc.order(datetime: direction))
               .pluck(:id)
          end
        ...
        context 'with case-INsensitive sort' do
          ...
          let(:expected_ids) do
            Foo.joins(:assoc)
               .merge(Assoc.order("LOWER(#{Assoc.table_name}.datetime) #{direction}"))
               .pluck(:id)
          end
