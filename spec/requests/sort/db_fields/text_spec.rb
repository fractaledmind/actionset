    ...
    context 'on the base object' do
      ...
      context 'when direction is ASC' do
        ...
        context 'with default case-sensitive sort' do
          let(:sort_param) { { 'text': direction } }
          let(:expected_ids) do
            Foo.order(text: direction)
               .pluck(:id)
          end
        ...
        context 'with case-INsensitive sort' do
          let(:sort_param) { { 'text(i)': direction } }
          let(:expected_ids) do
            Foo.order("LOWER(text) #{direction}")
               .pluck(:id)
          end
    ...
    context 'on an associated object' do
      ...
      context 'when direction is ASC' do
        ...
        context 'with default case-sensitive sort' do
          ...
          let(:expected_ids) do
            Foo.joins(:assoc)
               .merge(Assoc.order(text: direction))
               .pluck(:id)
          end
        ...
        context 'with case-INsensitive sort' do
          ...
          let(:expected_ids) do
            Foo.joins(:assoc)
               .merge(Assoc.order("LOWER(#{Assoc.table_name}.text) #{direction}"))
               .pluck(:id)
          end
