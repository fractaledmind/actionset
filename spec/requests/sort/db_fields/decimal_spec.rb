    ...
    context 'on the base object' do
      ...
      context 'when direction is ASC' do
        ...
        context 'with default case-sensitive sort' do
          let(:sort_param) { { 'decimal': direction } }
          let(:expected_ids) do
            Foo.order(decimal: direction)
               .pluck(:id)
          end
        ...
        context 'with case-INsensitive sort' do
          let(:sort_param) { { 'decimal(i)': direction } }
          let(:expected_ids) do
            Foo.order("LOWER(decimal) #{direction}")
               .pluck(:id)
          end
    ...
    context 'on an associated object' do
      ...
      context 'when direction is ASC' do
        ...
        context 'with default case-sensitive sort' do
          ...
          let(:expected_ids) do
            Foo.joins(:assoc)
               .merge(Assoc.order(decimal: direction))
               .pluck(:id)
          end
        ...
        context 'with case-INsensitive sort' do
          ...
          let(:expected_ids) do
            Foo.joins(:assoc)
               .merge(Assoc.order("LOWER(#{Assoc.table_name}.decimal) #{direction}"))
               .pluck(:id)
          end
