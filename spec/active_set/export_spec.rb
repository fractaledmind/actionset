...
RSpec.describe ActiveSet do
  ...
  describe '#export' do
    context 'with ActiveRecord collection' do
      ...
      context "{ format: :csv, columns: [{value: 'id'}, {value: 'only.string'}] }" do
        let(:instructions) do
          { format: :csv,
            columns: [
              { value: 'id' },
      ...
      context "{ format: :csv, columns: [{key: 'ID', value: 'id'}, {key: 'Assoc', value: 'only.string'}] }" do
        let(:instructions) do
          { format: :csv,
            columns: [
              { key: 'ID',
      ...
      context "{
        ...
        columns: [
          { key: 'ID',
            value: ->(item) { item.id * 2 } },
          { key: 'Assoc',
            value: ->(item) { item.only.string.upcase } }
        ] }" do
        let(:instructions) do
          { format: :csv,
            columns: [
        ...
        let(:expected_csv) do
          ::CSV.generate do |output|
            output << %w[ID Assoc]
            @active_set.each do |item|
              output << [(item.id * 2), item.only.string.upcase]
            end
          end
        end
